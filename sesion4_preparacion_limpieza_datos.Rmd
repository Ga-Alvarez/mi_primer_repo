...
filas_duplicadas_distintas <- datos_limpios |>  
  janitor::get_dupes() |> 
distinct() #para que  salgan los duplicados 
...
#crear otro mutate oara transformar los datos de la columna razon_encuentro
```
...
```{r}
filas_duplicadas_hospital<- datos_hospital |>  
  janitor::get_dupes() |> 
distinct()
```
...
alguna columna?
Eliminó 6 filas y 1 columna

```{r}
datos_sin_vacio_hospital <- datos_hospital |> 
...
```{r}
datos_hospital_reemplazo <- datos_sin_vacio_hospital |> 
  ...
  mutate(chills = case_when(chills == "Y" ~ "Yes",
                                  TRUE ~ chills,
                            cough == "Y" ~ "Yes",
                                  TRUE ~ cough,
                            aches == "Y" ~ "Yes",
