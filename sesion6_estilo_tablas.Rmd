...
ejercicio:
Añadir una nota para la observación 16988 que diga "El tamaño más grande"
...
```{r}
airquality_temp_altas <- datos_calidad_aire|> 
  arrange(desc(Temp)) |> 
  head(10)

airquality_temp_altas
```

```{r}
airquality_gt <- airquality_temp_altas|>  
  gt()
...
```{r}
airquality_gt <-airquality_gt|> 
   tab_header(
    title = "Calidad del aire en New York",
    subtitle = "Se presentan las 10 Temperatura más altas correspondientes al año 1973" 
  ) |> 
    tab_source_note(
  ...
  tab_footnote(
    footnote = "Mes con más temperaturas altas del top 10", 
    locations = cells_body(columns = Month, rows = 1:4))
```
...
```{r}
gtsave(airquality_gt,"arquality.png")
```
